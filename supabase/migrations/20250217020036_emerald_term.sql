/*
  ...
  1. Changes
    - Update balance calculation to properly handle bill payments
    - Add function to calculate total balance excluding unpaid bills
    - Add function to calculate monthly totals with proper bill handling
...
DECLARE
  total decimal;
BEGIN
  -- Calculate total from transactions, excluding transactions linked to unpaid bills
  SELECT COALESCE(SUM(
    CASE
  ...
  LEFT JOIN contas_pagar c ON t.conta_pagar_id = c.id
  WHERE t.user_id = user_id_param
    AND (start_date IS NULL OR t.data >= start_date)
    AND (end_date IS NULL OR t.data <= end_date)
    AND (t.conta_pagar_id IS NULL OR c.status = 'pago');
...
BEGIN
  ...
  WITH monthly_transactions AS (
    SELECT
      ...
      COALESCE(SUM(CASE 
        WHEN t.tipo = 'despesa' AND (t.conta_pagar_id IS NULL OR c.status = 'pago') 
        THEN t.valor 
        ELSE 0 
    ...
    LEFT JOIN contas_pagar c ON t.conta_pagar_id = c.id
    WHERE t.user_id = user_id_param
      AND t.data >= month_start
      AND t.data < month_end
  )
  SELECT
